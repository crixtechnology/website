// Builds a Crix Technology–branded PDF receipt entirely client-side (same
// approach as the standalone crix-billing-receipt tool this was ported
// from), fed by the auto-generated receipt record the backend returns from
// GET /api/receipts/:paymentId (see Backend/src/routes/receipts.js) instead
// of a hand-filled form. Company letterhead details come from src/data/
// content.js's `site` export — the single source of truth already used
// site-wide — rather than a second hardcoded copy.
...
// jsPDF's built-in Helvetica has no ₹ glyph — use an ASCII-safe prefix in
// the PDF itself (the on-screen UI can still show the real ₹ symbol).
const PDF_SYMBOL = "Rs. ";
...
/**
 * Builds the receipt PDF for one purchase. `receipt` is the object returned
 * by GET /api/receipts/:paymentId. Returns the jsPDF document.
 */
export async function buildReceiptPdf(receipt) {
  ...
  const logoData = await loadImageDataUrl("/crix-logo.png");
  ...
  // ── Status / payment mode strip — always PAID, this only exists for a
  // completed purchase (receipts.js only serves receipts on paid payments) ──
  ...
  doc.setFontSize(8.5);
  doc.setTextColor(...hexToRgb(BRAND.muted));
  const words = doc.splitTextToSize(`Amount in words: ${amountToWords(totalPaid, "INR")}`, CONTENT_W);
...
export async function downloadReceiptPdf(receipt) {
  ...
  const filename = `Receipt-${receipt.receiptNumber || "CRIX"}.pdf`.replace(/\s+/g, "-");
  doc.save(filename);
}
